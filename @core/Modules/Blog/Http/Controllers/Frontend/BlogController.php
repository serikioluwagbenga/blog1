...
class BlogController extends Controller
{
    ...
    public function archive_search(Request $request)
    {
           ...
           $query = $request->get('archive_search');
             $all_blogs = Blog::where('status', 'publish')
            ->whereDate('created_at', '<',Carbon::parse($query))
            ->paginate(10);

        return view(self::BASE_PATH.'blog.blog-search')->with([
            'all_blogs' => $all_blogs,
            'search_term' => __('Date wise search'),
...
HTML;
    ...
    public function author_profile($id)
    {
        ...
        return view(self::BASE_PATH.'blog.author-profile', compact('all_blogs', 'author_info'));
    }
