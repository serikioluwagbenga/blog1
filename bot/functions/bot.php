    ...
    function load_blog()
    {
        ...
        if (isset($data->articles->results)) {
            ...
            foreach ($bots as $bot) {
                ...
                $post['blog_content'] = '{"en_GB":"'.$this->RemoveSpecialChar($bot['body']).'"}';  
                ...
                $post['featured'] = "yes";
                $post['created_at'] = $this->date_format(strtotime($bot['dateTime']));
                $post['updated_at'] = $this->date_format(strtotime($bot['dateTime']));
                $post['status'] = "publish";
                $post['tag_id'] = '[""]';
